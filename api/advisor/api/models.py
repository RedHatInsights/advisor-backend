...
def get_host_group_filter(request, relation=None):
    """
    Filter hosts by RBAC workspace IDs and optional workspace names.  An empty
    workspace name matches hosts marked as ungrouped.
...
def stale_systems_q(org_id, field='host_id'):
    ...
    The literal org_id allows PostgreSQL to prune inventory partitions.
    """
    return Q(Exists(
...
class Host(ExportModelOperationsMixin('host'), TimestampedModel):
    ...
    """
    inventory_id = models.UUIDField(
        primary_key=True, serialize=False, db_column='system_uuid',
    )
    advisor_inventory = Relationship(
        'AdvisorInventoryHost',
