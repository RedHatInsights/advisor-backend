    ...
    view_template = f"""
        ...
        SELECT
            ...
            COALESCE(
                (COALESCE(
                    jsonb_build_object(
                        'bootc_status', s.bootc_status,
                        'host_type', s.host_type,
                        'operating_system', s.operating_system,
                        'owner_id', s.owner_id,
                        'rhc_client_id', s.rhc_client_id,
                        'system_update_method', s.system_update_method,
                        'rhsm', s.rhsm
                    ), 
                    '{{}}'::jsonb
                ) || COALESCE(
...
def check_or_create_subscription():
    ...
    try:
        ...
        hbi_host = hbi_config['host']
        hbi_port = hbi_config['port']
        hbi_db_name = hbi_config['name']
        hbi_user = hbi_config['user']
        hbi_password = hbi_config['password']
