...
- **AdvisorInventoryHost** — Local replica of HBI host data, populated by the Kafka inventory-events consumer. Uses `for_account(request)` manager method to scope queries by org_id and staleness
...
2. Create a filter function returning a `Q()` object, accepting an optional `relation` parameter for use in both `AdvisorInventoryHost` and `CurrentReport` queries
