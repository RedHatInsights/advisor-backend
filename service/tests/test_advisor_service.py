...
def test_autoacks_for_new_account(db, service, sample_engine_results, mock_request_post_return_200):
    ...
    # Both AdvisorInventoryHost rows for this account must exist
    assert models.AdvisorInventoryHost.objects.filter(account=account).count() == 2
...
@pytest.mark.django_db(transaction=True)
def test_handle_engine_results_reads_advisor_inventory_host(
    db, service, sample_engine_results, mock_request_post_return_200, mocker
):
...
@pytest.mark.django_db(transaction=True)
def test_handle_engine_results_advisor_host_not_found(
    db, service, sample_engine_results, mock_request_post_return_200, mocker
):
    """When no AdvisorInventoryHost record exists, the warning is logged and no webhook fires."""
