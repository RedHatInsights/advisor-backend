...
p = None
if kafka_settings.INVENTORY_VIEW_TOPIC:
    logger.debug(
        "Creating producer for inventory view topic %s.",
        kafka_settings.INVENTORY_VIEW_TOPIC,
    )
...
def send_inventory_view_event(event_data):
    ...
    """
    if p is None or not kafka_settings.INVENTORY_VIEW_TOPIC:
        return
    try:
        headers = [
        ...
        p.poll(0)
        p.produce(
            topic=kafka_settings.INVENTORY_VIEW_TOPIC,
            value=payload_str,
            headers=headers,
